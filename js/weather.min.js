...
this.retry():(f=b.data,$(".container").show(),$(".error").hide(),i18n.init(function(b){$(".container").i18n();$.each(f.getElementsByTagName("observation"),function(a,b){if(0!=a&&(q=b.getAttribute("icon_name"),null!=q&&"cw_no_report_icon"!=q))return d=this,!1});null==d&&(d=1<f.getElementsByTagName("observation").length?f.getElementsByTagName("observation")[1]:f.getElementsByTagName("observation")[0]);d&&"three-day"!=u&&(d.getAttribute("icon_name")?(a.loadImage(d.getAttribute("icon_name"),$(".current-icon")),
$(".current-icon").attr("title",d.getAttribute("description"))):$(".current-icon").hide(),$(".current-temp").html("celsius"==prefs.getString("unit")?a.convertTemp(d.getAttribute("temperature"))+"&#176;C":a.convertTemp(d.getAttribute("temperature"))+"&#176;F"));"custom"==t?($(".city").text(a.customDescription),n=!0):"service"==t&&(k=f.getElementsByTagName("location")[0])&&(l=k.getAttribute("city_name"),k.getAttribute("state_name")&&(l+=", "+k.getAttribute("state_name")),$(".city").text(l),n=!0);d&&
(prefs.getBool("show-humidity")?(r=i18n.t(d.getAttribute("wind_short")),p=prefs.getString("wind-speed"),isNaN(d.getAttribute("humidity"))||$(".humidity").text(i18n.t("humidity")+" "+d.getAttribute("humidity")+"%"),"mph"===p?d.getAttribute("wind_short")&&d.getAttribute("wind_speed")&&$(".wind").text(i18n.t("wind")+" "+r+" "+i18n.t("at")+" "+parseInt(d.getAttribute("wind_speed"))+" "+i18n.t("mph")):"kph"===p&&d.getAttribute("wind_short")&&d.getAttribute("wind_speed")&&(v=parseInt(d.getAttribute("wind_speed")),
